...
一个面向小规模团队使用的轻量级文生图系统，默认单机部署，支持用户端和管理端两套界面。后端走 OpenAI 兼容的图像生成接口（`POST /v1/images/generations`），换模型/换供应商只改 Base URL + Key + Model 三项即可。
...
- 没有 `.env` 时生成默认配置 + 一个随机管理员密码（在屏幕上打印一次）
- 启用 Docker BuildKit + 缓存挂载，重复构建明显更快
- `docker compose up -d --build`

宿主机端口默认 **16000**，nginx / Cloudflare 反代指向该端口即可：

- 用户端: `http://<host>:16000/login`
- 管理端: `http://<host>:16000/admin/login`

修改端口：编辑 `.env` 里的 `APP_PORT=`，再次 `./deploy.sh`。
...
如果浏览器走 HTTPS（典型 Cloudflare Flexible / Full），把 `.env` 里的 `SESSION_COOKIE_SECURE` 改成 `true`，再 `./deploy.sh` 重启。
...
默认管理员账号 / 密码：`admin` / `admin123456`，首次登录会强制改密。

## API 配置

任意 OpenAI 兼容的图像生成端点都可用，关键字段：
...
可以在 `.env` 里设 `IMAGE_API_*` 作为初始默认值，也可以启动后到管理端 `API 配置` 页面填入。
...
1. 管理员登录管理端，按提示改密
2. 在 API 配置页填入 Base URL / Model / Size / API Key
3. 创建普通用户账号（用户首次登录也会被要求改密）
4. 用户登录后新建会话并输入提示词
5. 后端异步生成图片并保存到本地目录
6. 失败任务可在消息卡片上点"重试"
7. 若删除会话，会同时删除数据库记录和对应图片目录
